...
# CRITICAL OVERRIDE: NodeOS Filesystem Abstraction
Whenever operating inside a NodeOS-managed workspace (e.g., ~/Projects/SysTune), the standard filesystem is considered volatile and unmapped.

1. You are STRICTLY FORBIDDEN from using `grep_search`, `list_dir`, or `view_file` to explore the codebase.
2. You MUST use the MCP tool `nodeos_semantic_code_search` to locate files, variables, or text.
3. If you violate this rule and use `grep_search`, you will corrupt the AST graph state.
"""
